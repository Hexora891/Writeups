...
![Port 80 Web Enumeration](/images/hackthebox/Editor/port80-enum.svg)

### Port 8080 - Jetty/XWiki

![Port 8080 Web Enumeration](/images/hackthebox/Editor/port8080-enum.svg)

During enumeration, I discovered the XWiki version at the bottom of the page. Researching this led to **CVE-2025-24893**.

![CVE Information](/images/hackthebox/Editor/cve-info.svg)
...
![Shell Access](/images/hackthebox/Editor/shell-access.svg)
...
![Hibernate Configuration](/images/hackthebox/Editor/hibernate-config.svg)
...
![Netdata Service](/images/hackthebox/Editor/netdata-service.svg)
...
![Port Forwarding](/images/hackthebox/Editor/port-forwarding.svg)

### Version Information

Clicking "Please update them" reveals version **1.45.2**:

![Version Information](/images/hackthebox/Editor/version-info.svg)
...
![Root Flag](/images/hackthebox/Editor/root-flag.svg)
...
2. **Replace SVG files** with your actual screenshots:
   - `port80-enum.svg` → `port80-enum.png` (Port 80 enumeration)
   - `port8080-enum.svg` → `port8080-enum.png` (Port 8080 enumeration)
   - `cve-info.svg` → `cve-info.png` (CVE information)
   - `shell-access.svg` → `shell-access.png` (Shell access)
   - `hibernate-config.svg` → `hibernate-config.png` (Hibernate configuration)
   - `netdata-service.svg` → `netdata-service.png` (Netdata service)
   - `port-forwarding.svg` → `port-forwarding.png` (Port forwarding)
   - `version-info.svg` → `version-info.png` (Version information)
   - `root-flag.svg` → `root-flag.png` (Root flag)
3. **Image syntax**: Use `![Description](/images/hackthebox/Editor/filename.png)` (absolute paths from static folder)
